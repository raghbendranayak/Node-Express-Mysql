...
function passwordVerification(req, res){
    ...
    var password = req.body.password;
    var queryPassword = "SELECT * FROM users WHERE email = 'admin@gmail.com' AND password = '123456'";

    var userData = con.query(queryPassword, function (err, result) {
   ...
   /* WebModel.user.findOne({'email': email,'role':2},{email:1, 'password': 1,'firstName':1,'role':1}, function(err, userData){
         if(userData!= null){
...
AdminController.changePassword = function(req, res) {
    ...
    if(newPassword === cpassword ){
        WebModel.user.update({ _id: userId}, {password: md5(newPassword)}, function (err, result) {
            if(err){
                console.log('');
...
AdminController.userListing = function(req, res) { console.log(req.session.userId);
    if(req.session.userId != "undefined" && req.session.userId != null){
        WebModel.user.find({'role': 3, status: 1 }, {firstName:1, lastName:1, email:1}, function (err, userlList) { 
            if(err) {
...
AdminController.editUser = function(req, res) { 
    if(req.session.userId != "undefined" && req.session.userId != null){
        WebModel.user.find({'_id': req.params.id }, {firstName:1, lastName:1}, function (err, userRecord) { 
            if(err) {
                req.flash('deleted', 'Something went wrong please try again.');
                res.redirect('/admin/users');
            }else{
...
AdminController.updateUser = function(req, res) { 
    if(req.session.userId != "undefined" && req.session.userId != null){
        ...
        WebModel.user.update({ _id: userToUpdateId }, {firstName:firstName, lastName:lastName}, function (err, result) { 
            if(err) {
...
AdminController.deleteUser = function(req, res) { 
    if(req.session.userId != "undefined" && req.session.userId != null){
        WebModel.user.find({'_id': req.params.id }, {firstName:1, lastName:1, email:1}, function (err, userData) { 
            if(err) {
                req.flash('deletedError', 'Something went wrong please try again.');
                res.redirect('/admin/users');
            }else{
                WebModel.user.update({ _id: req.params.id }, { status: 0 }, function (err, result) {
                    if(err){
                        req.flash('deletedError', 'Something went wrong please try again.');
                        res.redirect('/admin/users');
                    }else{
